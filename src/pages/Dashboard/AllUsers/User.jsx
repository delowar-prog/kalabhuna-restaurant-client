...
const User = ({ user, index, handleDeleteUser, handleMakeAdmin }) => {
    return (
        <tr>
            ...
            <td>{user.email}</td>
            <td>{
                user.role=='admin'? 'Admin': <button className="btn btn-warning btn-sm" onClick={()=>handleMakeAdmin(user)}><FaUserShield className="text-red-500 text-lg"></FaUserShield></button>
            }</td>
